...
const Signup: React.FC<SignupProps> = ({ onSwitch }) => {
  ...
  const handleSocialSignup = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      setError(e.message || `Failed to sign up with ${provider}.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md animate-fade-in">
      ...
      {error && (
        <div className="text-red-600 bg-red-50 text-sm mb-4 text-center p-3 rounded-md">
          {error}
        </div>
      )}
      
      {success && (
        <div className="text-green-600 bg-green-50 text-sm mb-4 text-center p-3 rounded-md">
          {success}
        </div>
      )}
